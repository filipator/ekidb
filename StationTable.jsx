...
export default function StationTable() {
  ...
  const filteredData = data.filter(item => {
    ...
    // Column filters
    const districtMatch = !filterConfig.District || 
      item.District.toLowerCase().includes(filterConfig.District.toLowerCase());
    
    const trainLineMatch = !filterConfig.TrainLine || 
      item.TrainLine.toLowerCase().includes(filterConfig.TrainLine.toLowerCase());
    
    return searchMatch && districtMatch && trainLineMatch;
  });
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-6">Train Stations Directory</h1>
      ...
      <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Filter by Train Line</label>
          <input
            type="text"
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {currentItems.map((station, index) => (
              ...
              >
                <td className="px-6 py-4 whitespace-nowrap">{station.District}</td>
                <td className="px-6 py-4 whitespace-nowrap">{station.TrainLine}</td>
                <td className="px-6 py-4 whitespace-nowrap">{station.StationJA}</td>
                <td className="px-6 py-4 whitespace-nowrap">{station.StationEN}</td>
      ...
      {selectedStation && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-3xl w-full p-6 relative">
            ...
            <h2 className="text-xl font-bold mb-2">{selectedStation.StationEN}</h2>
            <h3 className="text-lg mb-4">{selectedStation.StationJA}</h3>
